...
class AuthorUpdateRequest(BaseModel):
  is_reviewed: bool | None = Field(None, description="Set author reviewed")


@router.put("/{author_id}", response_model_exclude_none=True)
async def patch_author(
  ...
  try:
    author = await author_repo.update_author(author_id, request.is_reviewed)
  except BadDataRepositoryError:
    raise HTTPException(status_code=400, detail="Some values must be provided")
